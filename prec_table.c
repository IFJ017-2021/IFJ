...
int number_in_table(token_ptr token_table, bool a) {
  ...
  case T_DOUBLE:
    return 13;
  case T_K_NIL:
    if ((token_table->prev != NULL && token_table->prev->type != T_EQL &&
         token_table->prev->type != T_NEQL) ||
        (token_table->next != NULL && token_table->next->type != T_EQL &&
         token_table->next->type != T_NEQL)) {
...
void expression(DLList *list, bool where_expression) {
  ...
  while (prec_token->type != T_P_DOLLAR ||
         stack->array[stack->topIndex]->type != T_P_DOLLAR) {
    token_ptr tmp;
    Stack_Top(stack, &tmp);
