 ...
 */
void strFree(string *s) { free(s->data); }
...
int strAppendChar(string *s1, char toAppend) {
  if ((s1->length + 1) <
      s1->capacity) { // If there is enough of the allocated space
...
int strCopyString(string *s1, string *s2) {
  if (s2->length >= s1->capacity) { // If there is NOT enough memory allocated
    s1->data =
        (char *)realloc(s1->data, s2->length + 1); // Alloce enough space for s2
    if (s1->data == NULL) {
      return STR_ERR; // Allocation failed
    }
    s1->capacity =
        s2->length + 1; // Change capacity parametr of 's1'(destination string
                        // where we copy second string)
  }
...
int strCopyConstant(string *s1, char *c) {
  ...
  if (newLength >= s1->capacity) { // If there is NOT enough memory allocated
    s1->data = (char *)realloc(s1->data,
                               newLength + 1); // Alloce enough space for 's2'
    if (s1->data == NULL) {
      return STR_ERR; // Allocation failed
    }
    s1->capacity =
        newLength + 1; // Change capacity parametr of 's1'(destination string
                       // where we copy second string)
  }
 ...
 */
int strCompareConstant(string *s1, char *s2) { return strcmp(s1->data, s2); }

/**
 * @brief returns value of string 's'
 */
char *strGetString(string *s) { return s->data; }

/**
 * @brief returns lenght of string 's'
 */
int strGetLength(string *s) { return s->length; }

/**
 * @brief returns data size (capacity) of string 's'
 */
int strGetCapacity(string *s) { return s->capacity; }
